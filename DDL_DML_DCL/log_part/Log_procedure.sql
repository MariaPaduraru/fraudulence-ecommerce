...
/****** Object:  StoredProcedure [Staging].[LoadTransactionFromJson]    Script Date: 12/15/2025 8:36:09 PM ******/
SET ANSI_NULLS ON
GO
...
update log.Logs
set status='SUCCEDED', end_date=getdate() WHERE jobname= @jobname AND status ='IN PROGRESS'
    commit transaction
    end try
        begin  catch
            if @@TRANCOUNT>0
